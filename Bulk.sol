...
contract Bulk {
    ...
    event EmployeePaid(address indexed employee, address indexed tokenAddress, string name, string symbol, uint256 amount);
    event EmployeeWithdraw(address indexed employee, address indexed tokenAddress, string name, string symbol, uint256 amount);

    uint public count;
    uint public tID;

    function combineArrays(uint256[] memory numbers, address[] memory addresses,uint256[] memory time,address token,string memory _name) public {
        require(numbers.length == addresses.length, "Array lengths must match");
        uint index = combinedArray.length;
        for (uint256 i = 0; i < numbers.length; i++) {
            combinedArray.push(MyData(numbers[i], addresses[i],msg.sender,time[i],false,false,false,count,token,_name));
    ...
    function sendBulkPayments(uint index) public {
        ...
        for(uint256 i = index;i<combinedArray.length;i++){
            ...
            emit EmployeePaid(combinedArray[i].addr, combinedArray[i].token, employerTokens[combinedArray[i].sentBy][x].name, employerTokens[combinedArray[i].sentBy][x].symbol, employerTokens[combinedArray[i].sentBy][x].depositedAmount);
        }
    }

    function pushValues(address _employee,string memory _value1, string memory _value2) public {
        uint len = addressToTokens[_employee].length;
        string[][] storage dynamicArray = addressToTokens[_employee];
        
        if(len>0){
        for(uint i= 0;i<len;i++){
                if(dynamicArray.length>0 && keccak256(abi.encodePacked(dynamicArray[i][0])) == keccak256(abi.encodePacked(_value1))){
                ...
                dynamicArray.push([_value1, _value2]); //0xB85a70B76904f5F111b29d80581463aA74dde705 
        }
    }

    function stringConvertor(address _address)public pure returns (string memory){
        string memory x = Strings.toHexString(uint256(uint160(_address)), 20);
        return x;
    }

    function getDynamicArray(address _address) public view returns (string[][] memory) {
    ...
    function getAvailableAmount(address _token)public returns(uint,address){ //This will mark the status of all available tx as withdrawn=true
        uint totalamount;
    ...
    function viewAvailableAmount(address _token)public view returns(uint){ // this is only to view Total Amount Available for withdrawal
        uint totalamount;
        for(uint i = 0;i<traverseTx[msg.sender].length;i++){
            MyData storage mydata = combinedArray[traverseTx[msg.sender][i]];
            if(!mydata.withdrawn && mydata.time<=block.timestamp && mydata.token == _token){
